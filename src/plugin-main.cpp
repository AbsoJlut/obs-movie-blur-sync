...
struct Interval {
    ...
    {
        return seconds >= static_cast<double>(start) - preRoll && seconds <= static_cast<double>(end) + postRoll;
    }
};
...
std::optional<int> parseTimeToSeconds(const QString &value)
{
    ...
    if (parts.size() == 2) {
        ...
        if (!ok || ss >= 60) return std::nullopt;
        return mm * 60 + ss;
    }
    if (parts.size() == 3) {
        ...
        if (!ok || mm >= 60 || ss >= 60) return std::nullopt;
...
std::vector<Interval> parseIntervals(const QString &text, QString *error = nullptr)
{
    ...
    for (int i = 0; i < lines.size(); ++i) {
        ...
        if (!m.hasMatch()) {
            if (error) *error = QString("Строка %1: нужен формат 00:12-00:28 или 00:12:15-00:12:28").arg(i + 1);
            return {};
        }
        const auto start = parseTimeToSeconds(m.captured(1));
        const auto end = parseTimeToSeconds(m.captured(2));
        if (!start || !end) {
            if (error) *error = QString("Строка %1: не понял время").arg(i + 1);
            return {};
        }
        if (*end < *start) {
            if (error) *error = QString("Строка %1: конец раньше начала").arg(i + 1);
            return {};
        }
...
bool setSourceFilterEnabled(const QString &sourceName, const QString &filterName, bool enabled, QString *error = nullptr)
{
    obs_source_t *source = sourceByName(sourceName);
    if (!source) {
        if (error) *error = "Источник не найден: " + sourceName;
        return false;
    }
    obs_source_t *filter = obs_source_get_filter_by_name(source, filterName.toUtf8().constData());
    if (!filter) {
        if (error) *error = "Фильтр не найден: " + filterName;
...
private:
    void writeResponse(QTcpSocket *socket, const QByteArray &body, const char *contentType = "application/json", int code = 200, const char *text = "OK")
    {
        QByteArray header;
    ...
    void handleReadyRead(QTcpSocket *socket)
    {
        ...
        if (method != "POST" || path != "/push") {
            writeResponse(socket, R"({"ok":false,"error":"not_found"})", "application/json", 404, "Not Found");
            return;
        }

        const auto doc = QJsonDocument::fromJson(body);
        if (!doc.isObject()) {
            writeResponse(socket, R"({"ok":false,"error":"bad_json"})", "application/json", 400, "Bad Request");
            return;
        }
        ...
        if (!std::isfinite(current) || current < 0) {
            writeResponse(socket, R"({"ok":false,"error":"currentTime required"})", "application/json", 422, "Unprocessable Entity");
            return;
        }
        Snapshot snap;
        ...
        if (!std::isfinite(snap.playbackRate) || snap.playbackRate <= 0) snap.playbackRate = 1.0;
...
public:
    explicit MovieBlurDock(QWidget *parent = nullptr)
        : QDockWidget(QString::fromUtf8(kPluginName), parent), settings_("MovieBlurSync", "OBSPlugin")
    {
...
private:
    ...
    void buildUi()
    {
        ...
        preRoll_->setRange(0, 20); preRoll_->setDecimals(1); preRoll_->setSingleStep(0.1);
        postRoll_ = new QDoubleSpinBox(logicBox);
        postRoll_->setRange(0, 20); postRoll_->setDecimals(1); postRoll_->setSingleStep(0.1);
        scanInterval_ = new QDoubleSpinBox(logicBox);
        scanInterval_->setRange(0.2, 5); scanInterval_->setDecimals(1); scanInterval_->setSingleStep(0.1);
        maxHidden_ = new QDoubleSpinBox(logicBox);
        maxHidden_->setRange(0, 120); maxHidden_->setDecimals(1); maxHidden_->setSingleStep(1.0);
    ...
    void connectSignals()
    {
        ...
        connect(sourceCombo_, &QComboBox::currentTextChanged, this, [this]() { refreshFilters(); saveSettings(); });
        ...
        connect(postRoll_, qOverload<double>(&QDoubleSpinBox::valueChanged), this, &MovieBlurDock::saveSettings);
        connect(scanInterval_, qOverload<double>(&QDoubleSpinBox::valueChanged), this, &MovieBlurDock::saveSettings);
        connect(maxHidden_, qOverload<double>(&QDoubleSpinBox::valueChanged), this, &MovieBlurDock::saveSettings);
    ...
    void loadSettings()
    {
        ...
        intervalsEdit_->setPlainText(settings_.value("intervals", "00:12:15-00:12:28\n00:38:44-00:39:10\n01:04:10-01:05:00\n").toString());
    }
...
private slots:
    ...
    void refreshFilters()
    {
        const QString prev = settings_.value("blur_filter", filterCombo_->currentText().isEmpty() ? "Blur" : filterCombo_->currentText()).toString();
    ...
    void toggleBlurTest()
    {
        ...
        if (!setSourceFilterEnabled(sourceCombo_->currentText(), filterCombo_->currentText(), blurEnabled_, &error)) {
            lastError_ = error;
            blurEnabled_ = !blurEnabled_;
        }
        refreshStatus();
    ...
    {
        if (!snapshot_) {
            if (bridgeText) *bridgeText = "Bridge: нет данных от сайта";
            return std::nullopt;
        }
        const qint64 now = QDateTime::currentMSecsSinceEpoch();
        const double age = std::max(0.0, (now - snapshot_->receivedMs) / 1000.0);
        if (age > maxHidden_->value()) {
            if (bridgeText) *bridgeText = QString("Bridge: данные устарели %1 сек").arg(age, 0, 'f', 1);
            return std::nullopt;
        }
        ...
        if (title.isEmpty()) title = snapshot_->url;
        if (title.size() > 44) title = title.left(43) + "…";
        if (bridgeText) *bridgeText = QString("Bridge: %1 / %2 / %3").arg(formatSeconds(current), state, title.isEmpty() ? "video" : title);
        return current;
    }
    ...
    void tick()
    {
        ...
        if (desired != blurEnabled_) {
            if (setSourceFilterEnabled(sourceCombo_->currentText(), filterCombo_->currentText(), desired, &error)) {
    ...
    void refreshStatus()
    {
        ...
        if (running_) blur.prepend(paused_ ? "⏸ " : "🟢 ");
        if (!lastError_.isEmpty()) blur += " | Ошибка: " + lastError_;
